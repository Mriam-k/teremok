...
void DrawBackground ()
    ...
    txRectangle (2, 2, Window_X - 1, Window_Y - 2);
    ...
    txArc (1350, 380, Window_X, 490, 0, 180);
...
void DrawBear (int x, int y, double size, int eyes, int lips, int paw_l, int paw_r)
    {
    //лапы
    txSetColor     (RGB (64, 0, 0));
    txSetFillColor (RGB (64, 0, 0));

    txEllipse (x - 55 * size, y + 8  * size, x - 15 * size, y + 30 * size);
    txEllipse (x + 15 * size, y + 8  * size, x + 55 * size, y + 30 * size);
    txCircle  (x - 45 * size, y - 51 * size + paw_l, 12 * size);
    txCircle  (x + 45 * size, y - 51 * size + paw_r, 12 * size);

    //уши
    txCircle (x - 25 * size, y - 130 * size, 10 * size);
    txCircle (x + 25 * size, y - 130 * size, 10 * size);
    ...
    //голова
    txCircle  (x, y - 100 * size, 30 * size);

    //туловище
    txEllipse (x - 40 * size, y - 72 * size, x + 40 * size, y + 20 * size);

    txSetColor     (RGB (85, 0, 0));
    txSetFillColor (RGB (85, 0, 0));
    txEllipse (x - 25 * size, y - 62 * size, x + 25 * size, y + 10 * size);

    //морда
    txSetColor     (RGB (64, 0, 0));
    txSetFillColor (RGB (64, 0, 0));
    txEllipse (x - 20 * size, y - 105 * size, x + 20 * size, y - 75 * size);

    //глаза
    txSetColor     (RGB (0, 0, 128));
    txSetFillColor (RGB (0, 0, 128));
    txCircle (x - 13 * size, y - 107 * size, 6 * size);
    txCircle (x + 13 * size, y - 107 * size, 6 * size);

    txSetColor     (RGB (0, 0, 0));
    txSetFillColor (RGB (0, 0, 0));
    txCircle (x - 13 * size + eyes, y - 105 * size, 4 * size);
    txCircle (x + 13 * size + eyes, y - 105 * size, 4 * size);

    //нос
    txEllipse (x - 7 * size, y - 102 * size, x + 7 * size, y - 92 * size);

    //губы
    txSetColor     (RGB (155, 0, 0), lips);
    txSetFillColor (RGB (155, 0, 0));
    txArc (x - 10 * size, y - 100 * size, x + 10 * size, y - 83 * size, 180, 180);
    }
...
void MoveWolf_in_house ()
    ...
    while (t < 70)
        ...
        DrawCloud (t + 100,  100,  1.5);
        txTextOut (t + 300,  40,   "Забрался Волк в Теремок,");
...
void MoveCaptions_end ()
    ...
    while (t < 500)
        ...
        txRectangle (2, 2, Window_X - 2, Window_Y - 2);
